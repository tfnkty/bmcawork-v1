//----------------------------------------------------------
// THIS VERSION OF mlfunc.h OMITS ABSOLUTE INTENSITY AS DATA
//----------------------------------------------------------
...
void IDAlgorithmv2(arma::mat testdat) {
    // std::vector<double> testdat = {172.82, 659.102, 162.23, 207.74, 0}; // 137Cs, 192Ir
    ...
    for (size_t i = 0; i < testdat.size(); i += 1) {
...
void MLDecisionTreev2(arma::mat peakdata)
{
    mlpack::DecisionTree tree;
    mlpack::data::Load("DTv2.bin", "treev2", tree, true);
    size_t outputsp;
    arma::vec probssp;
    tree.Classify(peakdata, outputsp, probssp);
    std::cout << "Single Point ID: " << outputsp << std::endl;
}

void MLRandomForestv2(arma::mat peakdata)
{
    mlpack::RandomForest rf;
    mlpack::data::Load("RFv2.bin", "rfv2", rf, true);
    size_t outputsp;
    arma::vec probssp;
    rf.Classify(peakdata, outputsp, probssp);
    std::cout << "Single Point ID: " << outputsp << std::endl;
}

void MLHoeffdingTreev2(arma::mat peakdata)
{
    mlpack::HoeffdingTree ht;
    mlpack::data::Load("HTv2.bin", "htv2", ht, true);
    size_t outputsp;
    double probssp;
    ht.Classify(peakdata, outputsp, probssp);
    std::cout << "Single Point ID: " << outputsp << std::endl;
}

arma::mat VtoMv2(std::vector<double> v)
{
    return arma::conv_to<arma::mat>::from(v);
}

void MaterialIDv2(unsigned int *Spectrum)
{
